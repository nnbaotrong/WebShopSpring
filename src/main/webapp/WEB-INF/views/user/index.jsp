							...
							<div class="item active">
								<ul class="thumbnails">
									<c:forEach var="item" items="${n_products }" varStatus="loop">
										...
										<c:if test="${ (loop.index + 1) % 4 == 0 || (loop.index + 1)  == n_products.size() }">
								</ul>
						</div>
								<c:if test="${ (loop.index + 1) < n_products.size() }">
									<div class="item ">
								<ul class="thumbnails">
								</c:if>
						</c:if>
				...
				<c:if test="${ h_products.size() > 0 }">
					<ul class="thumbnails">
						...
						<c:forEach var="item" items="${ h_products }" varStatus="loop">
							...
							<c:if
								test="${ (loop.index + 1) % 3 == 0 || (loop.index + 1)  == h_products.size() }">
					</ul>
					<c:if test="${ (loop.index + 1) < h_products.size() }">
						<ul class="thumbnails">
					</c:if>
				</c:if>
